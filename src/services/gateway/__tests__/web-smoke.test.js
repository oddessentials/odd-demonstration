...
describe('Web Dashboard', () => {
    ...
    describe('Job Type Validation', () => {
        // Matches TUI validate_job_type()
        function validateJobType(jobType) {
            const trimmed = (jobType || '').trim();
            if (!trimmed) return 'Job type cannot be empty';
            if (trimmed.length > 50) return 'Job type too long (max 50 chars)';
            if (!/^[a-zA-Z0-9_-]+$/.test(trimmed)) return 'Job type must be alphanumeric';
            return null;
        }

        it('should reject empty job type', () => {
            expect(validateJobType('')).toBe('Job type cannot be empty');
            expect(validateJobType('   ')).toBe('Job type cannot be empty');
        });

        it('should reject job type exceeding 50 chars', () => {
            const longType = 'A'.repeat(51);
            expect(validateJobType(longType)).toContain('too long');
        });

        it('should reject special characters', () => {
            expect(validateJobType('job@type')).toContain('alphanumeric');
            expect(validateJobType('job type')).toContain('alphanumeric');
            expect(validateJobType('job.type')).toContain('alphanumeric');
        });

        it('should accept valid job types', () => {
            expect(validateJobType('PROCESS')).toBeNull();
            expect(validateJobType('my_job')).toBeNull();
            expect(validateJobType('job-123')).toBeNull();
            expect(validateJobType('Test_Job-1')).toBeNull();
        });
    });

    describe('Registry Validation', () => {
        // Matches TUI RegistryError validation
        function validateRegistry(registry) {
            if (!registry || typeof registry !== 'object') return 'Registry is not an object';
            if (!registry.baseUrl?.startsWith('http')) return 'baseUrl must start with http';
            if (!Array.isArray(registry.entries) || registry.entries.length === 0) return 'No entries';
            for (const entry of registry.entries) {
                if (!entry.id || !entry.name) return 'Entry missing id or name';
                if (entry.port <= 0 || entry.port > 65535) return 'Invalid port';
            }
            return null;
        }

        it('should reject missing baseUrl', () => {
            expect(validateRegistry({ entries: [{ id: 'x', name: 'X', port: 80 }] })).toContain('baseUrl');
        });

        it('should reject empty entries', () => {
            expect(validateRegistry({ baseUrl: 'http://localhost', entries: [] })).toContain('entries');
        });

        it('should reject invalid ports', () => {
            expect(validateRegistry({
                baseUrl: 'http://localhost',
                entries: [{ id: 'x', name: 'X', port: 0 }]
            })).toContain('port');
        });
        ...
        it('should accept valid registry', () => {
            expect(validateRegistry({
                baseUrl: 'http://localhost',
                entries: [{ id: 'dashboard', name: 'Dashboard', port: 8081 }]
            })).toBeNull();
        });
    });

    describe('Cross-Surface Invariants', () => {
        it('should use authoritative registry from contracts/', async () => {
            // This test validates that the registry file exists and has expected structure
            const fs = await import('fs');
            const path = await import('path');

            const registryPath = path.join(process.cwd(), '..', '..', '..', 'contracts', 'ui-registry.json');

            if (fs.existsSync(registryPath)) {
                const content = fs.readFileSync(registryPath, 'utf8');
                const registry = JSON.parse(content);

                // Verify structure matches what TUI and Web expect
                expect(registry).toHaveProperty('baseUrl');
                expect(registry).toHaveProperty('entries');
                expect(registry.baseUrl).toMatch(/^https?:\/\//);
